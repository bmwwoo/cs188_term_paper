...
When separate database servers were used (for example, in horizontal scaling), the database server was the same size as the application server (\texttt{db.m3med} was used for \texttt{m3.med}).
  ...
  \begin{figure}[H]
    ...
    Figure \ref{graph:vertical} shows the result of our vertical scaling. As expected, when double the procesing power was thrown at the problem, the number of users processed per second roughly doubled.
  ...
  Another method we implemented to spped up our application was to use a memory object caching system. We used memcached in our application through the "dalli" gem. The "dalli" gem stores the cache is stored in memory instead of on disk. Since memory is faster than disk, the read and write times of cache fragments were reduced. The memory cache appeared separately in each server. When the memory cache was full or the memory expired after a set amount of time, memcached flushed the old caches. By using memcached, the endpoint for the MySQL database was rarely hit as memcached was the first layer of data that a user should receive.\\

  We implemented memcached on the Yahoo! Finance data, the overall sentiment, and the average tweet.
\end{mainSection}
