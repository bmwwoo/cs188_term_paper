...
\subsection{Our goal}
The goal of Market Chirp is to quickly analyze the public's opinion of high-
quality stocks through short notes on Twitter. Since the feed of tweets is
...
indication to buy the stock. This is because the temperature of a company in the
eye of the public is often a good indicator of their stock's future performance
on the market. Market Chirp's ultimate goal is not to predict the prices of the
whole stock market, as the sentiment is based on a limited data set of people
tweeting about a stock, but to give a recommendation of whether to buy the stock
or not stock based on the public's opinion.
\\
...
master. By constantly rebasing our branches, our development history was clean
and looked as though only one person had developed it. Separating the features
...
our application much easier.  By using the following gems, we were able to
minimize the time spent developing and maximize the time spent scaling our
system. The functionality provided in external gems is more robust and has been
tested by others, improving the reliability of our services.
...
\end{figure}
  ...
  would have 5 tweets or fewer, making both the data inaccurate and the request
  ...
  \begin{figure}[H]
    ...
    Figure \ref{graph:vertical} shows the result of our vertical scaling tests.
    As expected, when double the processing power was thrown at the problem,
    the number of users processed per second roughly doubled. However, as we
    moved into the 2XLarge sized instance, we started to see some strange
    behavior. The close to linear improvement we had seen by just scaling the
    number of CPUs and memory of a server was no longer apparent. The drop in
    users per second seen in the 2XLarge instance can be explained by the
    request duration graph in figure \ref{graph:vertical_latency}. At about 50
    seconds into the test, we discovered that a race condition existed when two
    users attempted to query the same stock in the short period between
    updating and accessing the cache.
  ...
  \begin{figure}[H]
    ...
    These response code return rates show the improvement of the server when
    scaled vertically. While the rate of 503 errors improves only marginally,
    the rate of 200 codes increases in a near linear fashion. If we were to
    scale up our hardware past the capabilities of an m3.2XLarge instance
    however, we would probably start to see that the 200 rates only increase by
    a small amount.
  ...
  \ref{dig:horizontal}).  This network contained an elastic load balancer
  ...
  \begin{figure}[H]
    ...
    Below, we can also see that the request duration for each user also
    decreased in a proportionally similar manner. Longer response times
    correspond to delays that are visibly noticeable to a user, so we want to
    strike a balance between this latency and cost of additional servers. As we
    get closer to zero latency, we see the effects of diminishing returns on
    additional servers.\\
  ...
  \begin{figure}[H]
	...
	\pagebreak
    \subsubsection{HTTP response statistics}
    Here we lay out the results of the response code rates for both 200 and 503
    codes. We tested the improvements we got from vertically scaling the M3
    family of machines, as well as making use of horizontal scaling with
    Amazon's elastic load balancer. Using two machines of the same size, we
    received the output shown in the tables below.
	...
	\subsubsection{Combined Scaling}
	Using both vertical scaling and horizontal scaling, we set up a load
	balanced system consisting of two m3.2xlarge instances. The response in terms
	of users per second can be seen in Figure \ref{graph:horizontal}. In terms of
	simultaneous users that our app is able to handle, we also saw significant
	improvement. With our initial tests on the m3.med instance, we peaked at about
	800 concurrent users, however after scaling up to a 2XLarge instance and
	adding load balancing between two of these servers, we were able to handle up
	to 7400 users before the number of connected users began to diverge from the
  ...
  \subsubsection{Performance testing results}   We did our Memcached testing
  on a single \texttt{m3.Large} instance.  As shown in Figure
  \ref{graph:memcached}, Memcached let us serve more users per second, on
  average.  Figure \ref{graph:memcached_latency} shows the latency per request
  over the time of the tests.\\

  The little difference that Memcached made versus the MySQL caching method
  shows that the MySQL was not our main bottleneck. If our database were our
  main bottleneck, we should have seen a larger performance improvement by
  switching to Memcached, as it significantly reduced the number of database
  hits per page load.
  \begin{figure}[H]
    \begin{center}
      \begin{tikzpicture}
...
\begin{mainSection}{Conclusion}
  ...
  very application specific, however the same basic approach can be taken to get
  started in the right direction. These steps include ideas such as caching,
  vertical scaling, and horizontal scaling. Once these are taken care of, we
  have a tool set of techniques we can use to more selectively scale out an
  application. Examples of some techniques are query optimization, database
  sharding, in memory caching, and read or write slaves.\\
  ...
  \begin{itemize}
    \item Market Chirp scaled linearly with both more application servers and
      more processing power. Doubling either option lead to double the users
      served per second.
    ...
    \item Market Chirp was bottle necked more by CPU resources than database
