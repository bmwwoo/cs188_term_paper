...
\end{figure}
  ...
  \begin{figure}[H]
    ...
    \end{figure}
    Figure \ref{graph:horizontal} and \ref{graph:horizontal_latency} show the
    result of both horizontal and vertical scaling. As expected, when double
    the amount of servers were thrown at the problem, the number of users
    processed per second doubled.  Unlike vertical scaling, horizontal scaling
    is essentially limitless, and is usually cheaper. Below, we can also see
    that the request duration for each user also decreased in a proportionally
    similar manner. Longer response times correspond to delays that are visibly
    noticeable to a user, so we want to strike a balance between this latency
    and cost of additional servers. As we get closer to zero latency, we see
  ...
  \begin{figure}[H]
    ...
    After dealing with scaling our application to handle a large number of
    concurrent users, we could work toward achieving high-availability. This
    could involve using Amazon's RDS and Route 53 systems to have databases
    which cover different availability zones and regional domains.
