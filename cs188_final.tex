...
For all of our testing, we used a database that was seeded with 2,500 user accounts.  Each user had three favorites, with a fourth added during the test. This let our database have over 10,000 rows (especially after all of the company data was seeded).  For our Memcached test, discussed in depth in Section \ref{subsec:mem}, we used a database seeded with 25,000 user accounts for a database with over 100,000 rows.\\
...
\begin{mainSection}{Conclusion}
  ...
  \begin{itemize}
    ...
    \item Market Chirp is bottle necked more by CPU resources than database
  	  queries once caching is implemented.
